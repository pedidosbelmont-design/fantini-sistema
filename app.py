...
<style>
    ...
    .header-tabela {
        ...
        border-bottom: 3px solid #2c3e50; padding-bottom: 20px; margin-bottom: 30px;
    }
...
tab_gerador, tab_cadastro, tab_config = st.tabs(["📄 Gerador PDF", "📝 Cadastro", "⚙️ Ajustes"])

# --- ABA 1: GERADOR (CORRIGIDA - LINHA ÚNICA) ---
with tab_gerador:
    df_filtrado = df.copy()
    ...
    if df_filtrado.empty:
        ...
        if not itens_marcados.empty:
            ...
            # --- CORREÇÃO AQUI: GERAÇÃO EM LINHA ÚNICA ---
            html_rows = ""
            for idx, row in itens_marcados.iterrows():
                ...
                # TUDO NA MESMA LINHA PARA EVITAR ERRO DE ESPAÇAMENTO
                html_rows += f"<tr><td class='prod-cod' style='padding-left:15px;'>{cod_show}</td><td><div class='prod-nome'>{row['nome']}</div><div class='prod-ean'>{ean_show}</div></td><td style='text-align:right; padding-right:15px;' class='prod-preco'>R$ {preco:,.2f}</td></tr>"
            ...
            # HTML FINAL SEM ESPAÇOS NO INÍCIO DAS LINHAS IMPORTANTES
            html_final = f"""
...
"""
            ...
            st.success("✅ Gerado! Pressione Ctrl+P.")
        else:
